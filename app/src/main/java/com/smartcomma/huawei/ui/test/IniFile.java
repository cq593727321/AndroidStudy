...
public class IniFile {
	...
	public void save(String szOutFile) {
		OutputStream bufferedWriter;
		try {
			bufferedWriter = new FileOutputStream(szOutFile, false);
			saveConfig(bufferedWriter);
	...
	private void saveConfig(OutputStream bufferedWriter) {
		try {
			boolean line_spe = false;
			if (line_separator == null || line_separator.trim().equals(""))
				line_spe = true;
			for (Section section : sections.values()) {
				bufferedWriter.write(("[" + section.getName() + "]").getBytes());
				if (line_spe)
					bufferedWriter.write(line_separator.getBytes());
				else
					bufferedWriter.write("\n".getBytes());
				for (Map.Entry<String, Object> entry : section.getValues()
						.entrySet()) {
					bufferedWriter.write(entry.getKey().getBytes());
					bufferedWriter.write("=".getBytes());
					bufferedWriter.write(entry.getValue().toString().getBytes());
					if (line_spe)
						bufferedWriter.write(line_separator.getBytes());
					else
						bufferedWriter.write("\n".getBytes());
